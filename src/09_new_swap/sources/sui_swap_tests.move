...
/// |               +-- test_withdraw_all
/// ```
module new_swap::sui_swap_tests {
    use sui::sui::SUI;
    use sui::coin::{Self, Coin, mint_for_testing as mint};
    use sui::test_scenario::{Self as test, Scenario, next_tx, ctx};
    use new_swap::sui_swap::{Self, Pool, LSP};
    use sui::test_utils;
    ...
    fun test_init_pool_(test: &mut Scenario) {
        ...
        next_tx(test, owner);
        {
            sui_swap::init_for_testing(ctx(test));
        };

        next_tx(test, owner);
        {
            let lsp = sui_swap::create_pool(
                POOLEY {},
        ...
        next_tx(test, owner);
        {
            ...
            let (amt_sui, amt_tok, lsp_supply) = sui_swap::get_amounts(&pool);
    ...
    fun test_add_liquidity_(test: &mut Scenario) {
        ...
        next_tx(test, theguy);
        {
            ...
            let (amt_sui, amt_tok, lsp_supply) = sui_swap::get_amounts(pool_mut);

            let lsp_tokens = sui_swap::add_liquidity(
                pool_mut,
    ...
    fun test_swap_sui_(test: &mut Scenario) {
        ...
        next_tx(test, the_guy);
        {
            ...
            let token = sui_swap::swap_sui(pool_mut, mint<SUI>(5000000, ctx(test)), ctx(test));
    ...
    fun test_swap_tok_(test: &mut Scenario) {
        ...
        next_tx(test, owner);
        {
            ...
            let sui = sui_swap::swap_token(pool_mut, mint<BEEP>(1000, ctx(test)), ctx(test));
    ...
    fun test_withdraw_almost_all_(test: &mut Scenario) {
        ...
        next_tx(test, owner);
        {
            ...
            let (sui, tok) = sui_swap::remove_liquidity(pool_mut, lsp, ctx(test));
            let (sui_reserve, tok_reserve, lsp_supply) = sui_swap::get_amounts(pool_mut);
    ...
    fun test_math_(_: &mut Scenario) {
        ...
        // Try small values
        assert!(sui_swap::get_input_price(10, 1000, 1000, 0) == 9, 0);

        // Even with 0 commission there's this small loss of 1
        assert!(sui_swap::get_input_price(10000, max_val, max_val, 0) == 9999, 0);
        assert!(sui_swap::get_input_price(1000, max_val, max_val, 0) == 999, 0);
        assert!(sui_swap::get_input_price(100, max_val, max_val, 0) == 99, 0);
    }
