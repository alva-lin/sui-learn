module new_swap::sui_swap {
    ...
    public fun create_pool<P: drop, T> (
        ...
        assert!(sui_amt > 0 && token_amt > 0, EZeroAmount);
        assert!(sui_amt < MAX_POOL_VALUE && token_amt < MAX_POOL_VALUE, EPoolFull);
        assert!(fee_percent >= 0 && fee_percent < (FEE_SCALING as u64), EWrongFee);

        let share = math::sqrt(sui_amt) * math::sqrt(token_amt);
    ...
    public fun swap_sui<P, T> (
        ...
        let token_output_amount = get_input_price(
        ...
        coin::take(&mut pool.token, token_output_amount, ctx)
    }
    ...
    public fun swap_token<P, T>(
        ...
        let sui_output_amount = get_input_price(
        ...
        coin::take(&mut pool.sui, sui_output_amount, ctx)
    }
    ...
    public fun sui_price<P, T>(pool: &Pool<P, T>, token_to_sell: u64): u64 {
        let (sui_amt, token_amt, _) = get_amounts(pool);
        get_input_price(token_to_sell, token_amt, sui_amt, pool.fee_percent)
    }

    /// Public getter for the price of token T in SUI.
    /// - How much T one will get if they send `to_sell` amount of SUI;
    public fun token_price<P, T>(pool: &Pool<P, T>, sui_to_sell: u64): u64 {
        let (sui_amt, token_amt, _) = get_amounts(pool);
        get_input_price(sui_to_sell, sui_amt, token_amt, pool.fee_percent)
    }
    ...
    // 价格 = 输入数量 * 输出储备 / (输入储备 + 输入数量)
    public fun get_input_price_without_fee(
        input_amount: u64,
        input_reserve: u64,
        output_reserve: u64
    ): u64 {
